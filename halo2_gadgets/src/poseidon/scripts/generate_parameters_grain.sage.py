...
_sage_const_0 = Integer(0); _sage_const_160 = Integer(160); _sage_const_62 = Integer(62); _sage_const_51 = Integer(51); _sage_const_38 = Integer(38); _sage_const_23 = Integer(23); _sage_const_13 = Integer(13); _sage_const_2 = Integer(2); _sage_const_4 = Integer(4); _sage_const_12 = Integer(12); _sage_const_10 = Integer(10); _sage_const_1 = Integer(1); _sage_const_30 = Integer(30); _sage_const_64 = Integer(64); _sage_const_0xFFFF = Integer(0xFFFF); _sage_const_48 = Integer(48); _sage_const_16 = Integer(16); _sage_const_3 = Integer(3); _sage_const_6 = Integer(6); _sage_const_5 = Integer(5); _sage_const_7 = Integer(7)#!/usr/bin/env sage
...
def print_hex(c, last, rust=False):
    ...
    if rust:
        ...
        for i in range(_sage_const_0 , FIELD_SIZE, _sage_const_64 ):
            print("            0x%04x_%04x_%04x_%04x," % tuple([(c >> j) & _sage_const_0xFFFF  for j in range(i+_sage_const_48 , i-_sage_const_1 , -_sage_const_16 )]))
        print("        ]),")
    else:
        hex_length = (FIELD_SIZE + _sage_const_3 )//_sage_const_4  + _sage_const_2  # +2 for "0x"
...
def print_round_constants(round_constants, n, t, field, rust=False):
    ...
    assert len(round_constants) % t == _sage_const_0 
    rounds = len(round_constants) // t  # R_F + R_P
    for r in range(rounds):
        print("    [", end="\n" if rust else "")
        for (i, entry) in enumerate(round_constants[r*t : (r+_sage_const_1 )*t]):
            print_hex(entry, i == t-_sage_const_1 , rust=rust)
        print("    ]," if rust else "],")
...
def main(args):
    ...
    FIELD = int(args[_sage_const_0 ]) # 0 .. GF(2^n), 1 .. GF(p)
    SBOX = int(args[_sage_const_1 ]) # 0 .. x^alpha, 1 .. x^(-1)
    FIELD_SIZE = int(args[_sage_const_2 ]) # n
    NUM_CELLS = int(args[_sage_const_3 ]) # t
    R_F_FIXED = int(args[_sage_const_4 ])
    R_P_FIXED = int(args[_sage_const_5 ])

    PRIME_NUMBER = _sage_const_0 
    if FIELD == _sage_const_0 :
        args = args[_sage_const_6 :]
    elif FIELD == _sage_const_1  and len(args) < _sage_const_7 :
        print("Please specify a prime number (in hex format)!")
        return
    elif FIELD == _sage_const_1  and len(args) >= _sage_const_7 :
        PRIME_NUMBER = int(args[_sage_const_6 ], _sage_const_16 ) # e.g. 0xa7, 0xFFFFFFFFFFFFFEFF, 0xa1a42c3efd6dbfe08daa6041b36322ef
        args = args[_sage_const_7 :]
